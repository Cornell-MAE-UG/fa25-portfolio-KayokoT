...
ANSYS calculated a maximum normal stress of 67243 psi. Since this occurs where the clamped boundary condition ends, it is a mathematical stress singularity and likely not a realistic maximum. A more realistic maximum occurs as a stress concentration between the drive and beam.

![Maximum stress, neglecting singularity]({{ "truemax.png" | relative_url }}){: style="width: 200px; display: block; margin: 0 auto;" }

This indicates the maximum is 53984 psi, which is 98% higher than the calculated maximum stress of 18370 psi using beam theory. This is expected as the model does not account for stress concentrations. Using this true maximum, the torque wrench has a safety factor for strength of 2.7, which is too low to meet requirements. While I did not reiterate my design to account for this stress concentration, in practice it would be critical to either reduce stress concentrations through filleting, or selecting a stronger material. Notably, beam theory is highly accurate away from the clamped drive; at the gauge section, the expected stress was 17,219 psi, and ANSYS predicted 17,220 psi, showing nearly zero error.
...
At the strain gauge, ANSYS outputs a strain of 1069.6 microstrain, which has essentially zero error with the predicted value of 1069.53 microstrain. 

![Maximum stress, neglecting singularity]({{ "truemax.png" | relative_url }}){: style="width: 200px; display: block; margin: 0 auto;" }

Using the strain value from ANSYS, and assuming a half-bridge strain with a gauge calibration factor of 2, the torque wrench sensitivity equals 1.0696 mV/V. This meets the design requirement. 
